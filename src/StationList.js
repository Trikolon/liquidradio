...
export default class StationList {
   ...
   * @param {Array} source - Objects with src = url of audio stream and type to be injected into
   * audio element.
  ...
  getStationIndex(station) {
    if (station instanceof Station) {
      ...
      throw new Error('Invalid argument "station". Must either be string (station-id) or Station' +
        'object');
    }
  }
    ...
    stations.forEach((station) => {
      try {
        if (station instanceof Station) { // Either array contains station objects
          ...
          this.addStation(
            station.id, station.title, station.description,
            station.source, overwrite,
          );
        }
      } catch (e) {
        log.debug(e);
  ...
  load(overwrite = false) {
    if (localStorage) {
      ...
      if (storedStations) {
        ...
        // This ensures the format is correct. localStorage could contain invalid data;
        // Also it prevents duplicates and default stations from being overwritten
        if (!failed) {
          log.debug('Loaded stations object from localstorage', storedStations);

          // Parse into Station objects
          for (let i = 0; i < storedStations.length; i += 1) {
            try {
