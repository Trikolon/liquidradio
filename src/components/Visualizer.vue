...
<script>
  ...
  export default {
    ...
    methods: {
       ...
       */
      recalc() {
        // calculate an upper bound, we want to skip the highest frequencies in the last third of
        // the array
        ...
        log.debug(`fftSize: ${this.analyser.fftSize}; freqBytes length: ${this.freqBytes.length};
        upperbound: ${this.upperbound}; offset: ${this.offset}; cBarWith: ${this.cBarWith}`);
      },

      /**
       * checks on window resize if canvas dimensions have changed.
       * If so, update canvas client dimensions.
      ...
      drawSquare() {
        ...
        for (let i = 0; i < this.freqBytes.length; i += 1) {
          this.c.beginPath();
          ...
          this.c.lineTo(
            this.offset + (i * this.barWidth),
            (this.canvas.height - (((this.freqBytes[i] / 255) ** this.vertscale)
              * this.canvas.height)),
          );
          this.c.stroke();
        }
      ...
      drawCircle() {
        ...
        // at least 2 extra pixels to prevent shimmering
        this.c.lineWidth = Math.floor(this.cBarWith) + 2;

        for (let i = 0; i <= this.upperbound; i += 1) {
          const powered = (this.freqBytes[i] / 255) ** 2;
          // let powered = freqBytes[i] / 255; // To power, or not to power, that is the question

          // radius is minDim but take away half of linewidth, then decrease each step while going
          // through array. -1 was added in order to contain outer bars inside circle outline
          const radius = Math.max(this.minDim - 1 - (this.c.lineWidth / 2) -
            ((this.minDim / this.upperbound) * i), 0);

          // draw first half
          this.c.beginPath();
          this.c.arc(
            this.center[0], this.center[1], radius, 0.5 * Math.PI,
            (0.5 * Math.PI) + (Math.PI * powered), false,
          );
          this.c.stroke();
