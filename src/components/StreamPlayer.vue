<template>
    <div>
        <audio ref='audioEl' v-if='station' preload="metadata" crossorigin="anonymous">
            <source v-for="source in station.source" :key="source.src" :src="source.src"
                    :type="source.type">
        </audio>
    </div>
</template>
...
<script>
  ...
  export default {
    ...
    watch: {
      station() {
        // When station changes, re-attach error handler to new error element
    ...
    mounted() {
      ...
      if (AudioContext) {
        try {
          ...
          // for visualizer
          this.mediaElSrc = this.audioContext.createMediaElementSource(this.$refs.audioEl);
    ...
    methods: {
      /**
       * Attaches Listener to last source tag in html audio element and emits error event on error
       * from el
       * @returns {undefined}
       */
      ...
      /**
       * Called on error events of the audio element, emits error event with human readable message
       * for parent
      ...
      errorHandler(event) {
        ...
        const { networkState } = this.$refs.audioEl;
        // Check Network State
        if (networkState === 3) {
          message = 'Stream offline';
          this.offline = true;
        } else if (event.type === 'stalled') { // Check stalled property
          message = 'Network stalled';
        } else if (event.target.error && event.target.error.code) { // Check error codes
      ...
      reload(play = true) {
        ...
        if (play) {
          if (this.play) {
            ...
            // Stream play state was false before, set it to true (watcher will handle the dom play)
            this.play = true;
          }
        }
