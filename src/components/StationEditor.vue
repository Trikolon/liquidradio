    ...
    <div>
        <md-dialog ref='dialog' v-on:close="validationError=undefined">
            <md-dialog-title v-if="selectedStation">
                ...
                <md-button style="position: absolute; right: 10px; top: 10px;"
            ...
            <md-dialog-content v-if="selectedStation">
                ...
                <md-whiteframe v-for="source in selectedStation.source" key="{{source.src}}"
                               md-elevation="2"
                    ...
                    <md-button v-show="selectedStation.source.length > 1"
                               v-on:click="selectedStation.source
                               .splice(selectedStation.source.indexOf(source),1)"
                ...
                <md-dialog-actions>
                    <md-button v-show="!selectedStation.isNew" class="md-accent"
                               v-on:click="deleteHandler()">delete
                    </md-button>
  ...
  export default {
    ...
    methods: {
      ...
      addStation() {
        ...
        try {
          this.stations.addStation(
            this.selectedStation.id, this.selectedStation.title,
      ...
      deleteHandler() {
        try {
          // Station removed via id because selectedStation isn't a proper Station object
